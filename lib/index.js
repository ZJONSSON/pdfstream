...
class PdfStream extends Stream.PassThrough {
  ...
  /** @type { (inbound: Stream.Readable, name: string) => Promise<void> } */
  async append(inbound, name) {
    let firstPage;
    ...
    if (!firstLine.startsWith('%PDF')) {
      this.ended = true;
      pullStream.end();
      throw `File ${name} is not a zip file`;
    }
    ...
    while (!this.ended) {
      ...
      if (objMatch) {
        ...
        const res = await this.processObject(objNum, version, body);
  ...
  /** @type { (objNum: number, version: number, body: Buffer) => Promise<{firstPage?: number}> } */
  async processObject(objNum, version, body) {
    let firstPage, re;
    ...
    if (
      ...
      for (let i = 0; i < references.length; i++) {
        ...
        const res = await this.processObject(d.objNum, 0, payload);
        firstPage = firstPage || res.firstPage;
      }
    }
    ...
    if (obj.includes('/Pages') && !obj.includes('/Parent') && !obj.includes('/Catalog')) {
      ...
      this.pages.push({objNum, version, name: this.name});
